...
- [ ] `MCP_INTROSPECTION_SECRET` set (deploy.sh generates it) and the MCP
      subdomain answers `401` on `POST /mcp` without a token (see §9)
- [ ] Backups scheduled
...
2. `.env.production`: set `MCP_PUBLIC_URL`, `MCP_AUTHORIZATION_SERVER_URL` and
   `MCP_API_URL` for your domain. `MCP_INTROSPECTION_SECRET` is generated by
   `deploy.sh` and read by BOTH the app and the MCP container — rotating it
   disconnects every connected client.
3. Host proxy — nginx: `sudo cp deploy/nginx-host-mcp.conf
   /etc/nginx/sites-available/postsider-mcp`, edit the domain, enable it,
   `sudo nginx -t && sudo systemctl reload nginx`, then
   `sudo certbot --nginx -d mcp.example.com`. Caddy: the `mcp.example.com` block
   is already in `deploy/Caddyfile`.
