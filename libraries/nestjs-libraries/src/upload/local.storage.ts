...
import { withRemoteMediaPermit } from '@postsider/nestjs-libraries/upload/remote.response';
...
export class LocalStorage implements IUploadProvider {
  ...
  async uploadSimple(input: string) {
    ...
    const upload = async (body: Buffer) => {
      ...
      if (body.byteLength > maxUploadBytesForMime(detected.mime)) {
    ...
    if (dataUrl) {
      return upload(dataUrl.buffer);
    }

    return withRemoteMediaPermit(async (readResponse) => {
      ...
      return upload(await readResponse(loadImage));
    });
  }
