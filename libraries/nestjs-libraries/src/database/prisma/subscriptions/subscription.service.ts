...
export class MonthlyPostQuotaExceededError extends Error {}
...
export class SubscriptionService {
  ...
  async deleteSubscriptionByOrganizationId(organizationId: string) {
    const subscription =
      await this._subscriptionRepository.getSubscriptionByOrgId(organizationId);
    if (subscription?.isLifetime) {
      return;
    }

    await this._integrationService.changeActiveCron(organizationId);
    return this._subscriptionRepository.deleteSubscriptionByOrgId(organizationId);
  }

  async deleteSubscriptionByOrganizationIdIfCurrent(
    organizationId: string,
    identifier: string
  ) {
    return this._subscriptionRepository.deleteSubscriptionByOrgIdIfCurrent(
      organizationId,
      identifier
    );
  }
  ...
  async createOrUpdateSubscription(
    ...
    // A canceled-then-resubscribed org has its non-superadmin members disabled
    // by the cancellation path. modifySubscriptionByOrg only re-enables them on
    // a tier TRANSITION into team_members, so a same-tier re-subscribe (TEAM ->
    // TEAM) would leave the org permanently locked out. Re-enable whenever the
    // upserted tier grants team seats (mirroring modifySubscriptionByOrg).
  ...
  async reserveMonthlyPostSlots(organizationId: string, amount: number) {
    if (!amount || !isBillingEnabled()) return null;
    const subscription = await this.getSubscriptionByOrganizationId(organizationId);
    const tier = subscription?.subscriptionTier || 'FREE';
    const limit = pricing[tier]?.posts_per_month ?? 0;
    const createdAt = subscription?.createdAt || new Date();
    const monthsElapsed = Math.abs(dayjs(createdAt).diff(dayjs(), 'month'));
    const periodStart = dayjs(createdAt).add(monthsElapsed, 'month').toDate();
    const reservation = await this._subscriptionRepository.reserveMonthlyPostSlots(
      organizationId,
      periodStart,
      amount,
      limit,
    );
    if (!reservation) throw new MonthlyPostQuotaExceededError('Monthly post limit reached');
    return reservation.id;
  }

  releaseMonthlyPostReservation(id: string | null) {
    return id ? this._subscriptionRepository.refundCredit(id) : undefined;
  }
