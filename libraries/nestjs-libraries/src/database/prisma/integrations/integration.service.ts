...
export class IntegrationService {
  ...
  refreshSucceededIfCurrent(
    ...
    return this._integrationRepository.refreshSucceededIfCurrent(
      org,
      id,
      revision,
      refresh
  ...
  refreshFailedIfCurrent(org: string, id: string, revision: number) {
    return this._integrationRepository.refreshFailedIfCurrent(
      org,
      id,
      revision
    );
  ...
  refreshExtensionCredentialsIfCurrent(
    org: string,
    id: string,
    revision: number,
    accessToken: string,
    ...
    return this._integrationRepository.refreshExtensionCredentialsIfCurrent(
      org,
      id,
      revision,
      accessToken,
  ...
  refreshLinkedTokensIfCurrent(
    ...
    return this._integrationRepository.refreshLinkedTokensIfCurrent(
      org,
      id,
      rootInternalId,
  ...
  getLinkedTokenSiblingsForRefresh(
    ...
    return this._integrationRepository.getLinkedTokenSiblingsForRefresh(
      org,
      id,
      rootInternalId,
      refreshToken
  ...
  async setBetweenRefreshStepsIfCurrent(
    ...
    return this._integrationRepository.setBetweenRefreshStepsIfCurrent(
      org,
      id,
      revision
    );
  ...
  async refreshTokens() {
    ...
    for (const integration of integrations) {
      ...
      const siblings = provider.oneTimeToken
        ? await this.getLinkedTokenSiblingsForRefresh(
      ...
      if (!data) {
        const markedFailed = await this.refreshFailedIfCurrent(
          integration.organizationId,
          integration.id,
      ...
      const persisted = await this.refreshSucceededIfCurrent(
        integration.organizationId,
        integration.id,
        integration.revision,
        { accessToken, refreshToken, expiresIn }
      );
      if (persisted && provider.oneTimeToken) {
        await this.refreshLinkedTokensIfCurrent(
          integration.organizationId,
          integration.id,
          integration.rootInternalId,
          integration.refreshToken,
  ...
  async enableChannel(
    ...
    const integration = await this._integrationRepository.enableChannel(
      org,
      id,
      activeChannelLimit
    );
  ...
  async saveProviderPage(org: string, id: string, data: any) {
    ...
    const completed = await this._integrationRepository.completeProviderPageIfCurrent(
      org,
      id,
      getIntegration.revision,
      {
    ...
    if (!completed) {
      throw new HttpException(
        'Channel setup changed, please reconnect and select the page again',
        HttpStatus.CONFLICT
      );
    }
