...
MCP (Model Context Protocol) server that gives compatible AI clients selected
access to a PostSider instance through its public API. It authenticates with an
...
claude mcp add postsider \
  ...
  --env POSTSIDER_API_URL=https://social.example.com/api \
  ...
  "mcpServers": {
    "postsider": {
      ...
      "env": {
        ...
        "POSTSIDER_API_URL": "https://social.example.com/api"
      }
    }
  }
}
...
| `POSTSIDER_API_URL` | yes | - | Instance base URL, including `/api` behind the bundled proxy |
