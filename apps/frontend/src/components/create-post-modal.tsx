...
function stableSettings(
  ...
  return JSON.stringify(
    channels.map((id) => [
      id,
      // `commercial_content` is a UI-only mirror of the disclosure switch (the
      // transport strips it), so it must not make an untouched edit "dirty".
      Object.keys(settings[id] ?? {})
        .filter((k) => k !== "commercial_content")
        .sort()
...
function cleanValidationMessage(raw: string, t: Translate): string {
  ...
  // TikTok server-side validation is surfaced as English strings from the
  // provider — localize the known ones via the i18n catalogs (en/pl).
  const localized = localizeTiktokMessage(t, m);
  return localized === m ? capitalizeFirst(m) : localized;
}

/**
 * Map the TikTok validation strings the backend provider / composer validator
 * produce to i18n keys, so the composer shows them in the user's language.
 * Unknown strings pass through unchanged (other providers, future messages).
 */
function localizeTiktokMessage(t: Translate, msg: string): string {
  const trimmed = (msg || "").trim().replace(/[.!]+$/, "").trim();
  const duration = trimmed.match(
    /^This video is (\d+) seconds long\. This TikTok account allows videos up to (\d+) seconds$/i,
  );
  if (duration) {
    return t("createPost.tiktok.videoTooLong", {
      seconds: duration[1],
      max: duration[2],
    });
  }
  ...
  const map: Array<[RegExp, string]> = [
    ...
    [/^TikTok has not approved this app for public posting yet\. Until the review passes, the TikTok account must be set to private and posts can use Self only visibility$/i, "unaudited"],
    ...
    [/^Comments are turned off on this TikTok account$/i, "commentOff"],
    [/^Branded content can't be published with Self only visibility.*$/i, "commercialPrivate"],
    [/^Add a video or at least one photo — TikTok can't post without media$/i, "addVideoOrPhoto"],
    [/^A video post can only contain one media item — use photos only when selecting multiple items$/i, "videoPostOneItem"],
    [/^Choose who can see this post on TikTok$/i, "choosePrivacy"],
    [/^Choose the applicable content disclosure before posting to TikTok$/i, "chooseDisclosure"],
    [/^Only pictures are supported when selecting multiple items$/i, "onlyPictures"],
    [/^You need one media$/i, "needOneMedia"],
    [/^No video \/ images selected$/i, "noMedia"],
  ];
  for (const [re, key] of map) {
    if (re.test(trimmed)) {
      return t(`createPost.tiktok.${key}` as MessageKey);
    }
  }
  return msg;
}
...
export function CreatePostModal({
   ...
   *
   * Provider defaults (Instagram `post_type`, TikTok direct-post transport, ...) are
  ...
  useEffect(() => {
    const prune = (source: Record<string, Record<string, unknown>>) => {
      ...
      for (const [id, info] of Object.entries(tiktokCreators)) {
        ...
        if (!info) continue;
        const currentSettings = next[id] ?? {};
        let updated = currentSettings;
        if (current && !info.privacyOptions.includes(current)) {
          const { privacy_level: _dropped, ...rest } = next[id]!;
          updated = rest;
        }
        // Never submit a value the creator has disabled. Rendering the control
        // as unchecked alone would leave a stale `true` in the API payload.
        for (const [key, disabled] of [
          ["duet", info.duetDisabled],
          ["stitch", info.stitchDisabled],
          ["comment", info.commentDisabled],
        ] as const) {
          if (disabled && updated[key] !== false) {
            updated = { ...updated, [key]: false };
          }
        }
  ...
  // fields (e.g. TikTok direct-post transport, Instagram post_type) are always present
  ...
  const buildPost = useCallback((): NewPostInput => {
    ...
    const perChannelSettings = Object.fromEntries(
      Object.entries(channelSettings).map(([channelId, settings]) => {
  ...
  const channelProblems = useMemo<Record<string, string[]>>(() => {
    ...
    for (const c of selectedChannels) {
      ...
      let problems = req.validate({
        ...
        body,
        commentsHaveMedia,
        maxVideoDurationSeconds: tiktokCreators[c.id]?.maxDurationSeconds,
      });
      // TikTok validation messages come from the shared requirement registry
      // in English — localize them (and the backend's creator-derived ones)
      // through the i18n catalogs for this channel only.
      if (channelIdentifier(c) === "tiktok") {
        problems = problems.map((p) => localizeTiktokMessage(t, p));
      }
      ...
      // Required settings fields that are still empty. Only fields the panel
      // would actually render count (hidden fields and media-conditional
      // fields for the current media mode are skipped) — otherwise a TikTok
      // post with no attachment would demand privacy for an invisible select.
      const settings = channelSettings[c.id] ?? {};
      const tiktokMediaMode =
        channelIdentifier(c) === "tiktok"
          ? mediaLike.length === 0
            ? "none"
            : mediaLike.some((m) => m.kind === "video")
              ? "video"
              : "photo"
          : null;
      for (const field of req.fields) {
        ...
        if (field.showIf && !field.showIf(settings)) continue;
        if (tiktokMediaMode !== null && field.showForMedia) {
          if (
            (field.showForMedia === "attached" && tiktokMediaMode === "none") ||
            (field.showForMedia !== "attached" &&
              field.showForMedia !== tiktokMediaMode)
          ) {
            continue;
          }
        }
  ...
  // TikTok audit rule: when Content Disclosure is ON but no type is chosen,
  // publishing/scheduling must be blocked (not just warned) — the reviewer
  // checks that Publish now is visibly disabled in exactly this state.
  const blocksTiktokPublish = useMemo(() => {
    for (const c of selectedChannels) {
      const req = requirementFor(c);
      if (!req || req.identifier !== "tiktok") continue;
          ...
          <div className={styles.footRight}>
            ...
            {!isEdit && onSendToApproval && (
              <button
                type="button"
                className={styles.btnSecondary}
                disabled={submitting !== null || blocksTiktokPublish}
            ...
            {selectedChannels.length === 1 && (
              <button
                type="button"
                className={styles.btnSecondary}
                disabled={submitting !== null || addingToQueue || blocksTiktokPublish}
                onClick={addToQueue}
              >
                {addingToQueue
...
interface ProviderSettingsPanelProps {
  ...
  /** Current attachments, used for TikTok's photo/video-specific controls. */
  media: MediaLike[];
...
function ProviderSettingsPanel({
  ...
  const visibleFields = requirement.fields.filter(
    (field) =>
      !field.hidden &&
      (!field.showIf || field.showIf(settings)) &&
      (!field.showForMedia ||
        (field.showForMedia === "attached" && tiktokMediaMode !== "none") ||
        field.showForMedia === tiktokMediaMode),
  );
  ...
  const hasBranded = Boolean(settings.brand_content_toggle);
  const disclosureEnabled = Boolean(settings.commercial_content) || commercial;
  ...
  return (
    <div className={styles.providerPanel}>
      ...
      {isTiktok && creatorInfo && (
        <div className={styles.tiktokPostingAs}>
          <span className={styles.tiktokPostingAsLabel}>
            {t("createPost.tiktok.postingAs")}
          </span>
          {creatorInfo.avatarUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={creatorInfo.avatarUrl} alt="" className={styles.tiktokCreatorAvatar} />
          ) : (
            <span className={styles.tiktokCreatorAvatar} aria-hidden />
          )}
          <span className={styles.tiktokCreatorName}>
            {creatorInfo.nickname || creatorInfo.username || channel.name}
          </span>
          {creatorInfo.username && (
            <span className={styles.tiktokCreatorUsername}>@{creatorInfo.username}</span>
          )}
          {creatorInfo.maxDurationSeconds > 0 && tiktokMediaMode === "video" && (
            <span className={styles.tiktokDuration}>
              {t("createPost.tiktok.maxVideoDuration", {
                seconds: creatorInfo.maxDurationSeconds,
              })}
            </span>
          )}
        </div>
      ...
      {isTiktok && tiktokMediaMode !== "none" && (
        <div className={styles.tiktokDisclosure}>
          <div className={styles.tiktokDisclosureRow}>
            ...
            >
              <input
                ...
                onChange={(e) => onChange("commercial_content", e.target.checked)}
              />
              <span className={styles.tiktokSwitchTrack} aria-hidden />
              <span className={styles.tiktokSwitchState}>
                {disclosureEnabled
                  ? t("createPost.tiktok.disclosureOn")
                  : t("createPost.tiktok.disclosureOff")}
              </span>
            </label>
          </div>
          {disclosureEnabled && (
            <div className={styles.tiktokDisclosureChildren}>
              <span className={styles.fieldHelp}>
                {t("createPost.tiktok.disclosureHelp")}
              </span>
              <label className={styles.fieldCheckbox}>
                <input
                  type="checkbox"
                  checked={Boolean(settings.brand_organic_toggle)}
                  onChange={(e) => onChange("brand_organic_toggle", e.target.checked)}
                />
                <span>
                  {t("createPost.tiktok.yourBrand")}
                  <span className={styles.tiktokOptionHelp}>
                    {t("createPost.tiktok.yourBrandHelp")}
                  </span>
                </span>
              </label>
              <label className={styles.fieldCheckbox}>
                <input
                  ...
                  onChange={(e) => onChange("brand_content_toggle", e.target.checked)}
                />
                <span>
                  {t("createPost.tiktok.brandedContent")}
                  <span className={styles.tiktokOptionHelp}>
                    {settings.privacy_level === "SELF_ONLY"
                      ? t("createPost.tiktok.brandedPrivate")
                      : t("createPost.tiktok.brandedContentHelp")}
                  </span>
                </span>
              </label>
            </div>
          ...
          {commercial && (
            <div className={styles.tiktokDisclosureResult}>
              {Boolean(settings.brand_content_toggle)
                ? tiktokMediaMode === "video"
...
interface ProviderFieldProps {
  ...
  /** TikTok only: what the connected account allows. */
  creatorInfo?: TiktokCreatorInfo;
  /** TikTok Branded content cannot use the SELF_ONLY privacy level. */
  branded?: boolean;
          ...
          {needsExplicitChoice && (
            ...
            {options.map((opt) => (
              <option
                key={opt.value}
                value={opt.value}
                disabled={branded && opt.value === "SELF_ONLY"}
              >
              {opt.label}
            </option>
          ))}
