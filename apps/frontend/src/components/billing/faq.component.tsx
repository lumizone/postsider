  ...
  return [
    ...(user?.allowTrial
      ? [
          {
            title: t(
              'faq_am_i_going_to_be_charged_by_postsider',
              'Am I going to be charged by Postsider?'
            ),
            description: t(
              'faq_to_confirm_credit_card_information_postsider_will_hold',
              'To confirm credit card information Postsider will hold $2 and release it immediately, you can cancel your subscription anytime from settings without talking to a person'
            ),
          },
        ]
      : []),
    {
      title: t(
        'faq_can_i_trust_postsider_gitroom',
        `Can I trust ${isGeneral ? 'Postsider' : 'Gitroom'}?`
      ),
      description: t(
        'faq_postsider_gitroom_is_proudly_open_source',
        `${
          isGeneral ? 'Postsider' : 'Gitroom'
        } is proudly open-source! We believe in an ethical and transparent culture, meaning that ${
          isGeneral ? 'Postsider' : 'Gitroom'
        } will live forever. You can check out the entire code or use it for personal projects. To view the open-source repository, <a href="https://github.com/gitroomhq/postsider-app" target="_blank" style="text-decoration: underline;">click here</a>.`
      ),
    },
    {
      title: t('faq_what_are_channels', 'What are channels?'),
      description: t(
        'faq_postsider_gitroom_allows_you_to_schedule_posts',
        `${
          isGeneral ? 'Postsider' : 'Gitroom'
