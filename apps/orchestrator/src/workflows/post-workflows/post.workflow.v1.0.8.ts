/**
 * Publish workflow v1.0.8 — fire-time state check.
 *
 * v1.0.7 validated QUEUE + the Emergency Pause gate at workflow START, then
 * slept until publishDate and published without re-checking. A scheduled post
 * edited back to DRAFT/HELD while the workflow slept would still publish
 * (observed in production 2026-08-30: four DRAFT posts with running V106/V107
 * executions, one minutes from its due time). v1.0.8 sleeps FIRST, then
 * re-loads the post and re-validates QUEUE + the pause gate AT fire time, so
 * an unscheduled draft stays a draft.
 *
 * Everything else is behavior-identical to v1.0.7 (token-free *ById activities,
 * per-metric fallbacks, retry semantics). v1.0.7 stays untouched — running
 * workflows replay against the code they started with.
 */
...
export async function postWorkflowV108({
  ...
  // Load once purely to compute how long to sleep. The authoritative state
  // check happens AGAIN at fire time below, after the sleep.
  const preFlight = await getPostForPublish(organizationId, postId);
  if (!preFlight) {
  ...
  if (!postNow) {
    await sleep(
      dayjs(preFlight.publishDate).isBefore(dayjs())
        ? 0
        : dayjs(preFlight.publishDate).diff(dayjs(), 'millisecond')
    );
  }

  // ── FIRE TIME ───────────────────────────────────────────────────────────
  // Re-load and re-validate NOW. A post that was edited back to DRAFT/HELD
  // (or published by a replacement workflow) while this workflow slept must
  // not fire. The pause gate below is atomic with the DB.
  ...
  if (!postNow && firstPost.state !== 'QUEUE') {
    // Deliberate user action (unscheduled draft, held by pause, or already
    // published by a replacement workflow) — stop silently, not an error.
    return;
  }
