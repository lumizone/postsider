...
export class PostActivity {
  ...
  async refreshTokenById(
    ...
    const integration = await this._loadIntegration(orgId, integrationId);
    // The workflow read this revision before scheduling the activity. A newer
    // reconnect/disable/delete must win without another provider call.
    if (
      integration.revision !== (revision ?? integration.revision) ||
