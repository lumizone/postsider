...
import { ChannelCapacityExceededError } from '@postsider/nestjs-libraries/database/prisma/integrations/integration.repository';
...
export class NoAuthIntegrationsController {
  ...
  async connectSocialMedia(
    ...
    // with the plan limit fully unenforced. The repository now makes the final
    // active-channel decision transactionally, so reconnects only bypass the
    // limit when they truly keep an already-active row active.
    const activeChannelLimit = isBillingEnabled()
      ? (await this._permissionsService.getPackageOptions(org.id)).subscription
          ?.totalChannels || 0
      : undefined;

    let createUpdate;
    try {
      createUpdate = await this._integrationService.createOrUpdateIntegration(
        additionalSettings,
      ...
      if (error instanceof ChannelCapacityExceededError) {
  ...
  async extensionRefreshCookies(
    ...
    if (String(authResult.id) !== String(integration.internalId)) {
      // The extension JWT may describe a channel that has since been
      // reconnected. Only the revision authenticated above may mark it stale.
      await this._integrationService.refreshFailedIfCurrent(
        organizationId,
        integrationId,
    ...
    const persisted = await this._integrationService.refreshExtensionCredentialsIfCurrent(
      organizationId,
      integrationId,
      integration.revision,
    ...
    if (!persisted) {
      // Disable/delete can race the extension's provider request. The CAS
      // keeps that lifecycle decision authoritative rather than reviving it.
      throw new HttpException('Integration not found', 404);
    }
